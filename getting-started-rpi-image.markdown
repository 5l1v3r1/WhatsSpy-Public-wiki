...
*Tested on Rpi Model 1B (4GB) and 2B (16GB) and one report of working QEMU working. You can also support me by buying a Raspberry Pi 2B via this [Amazon link](http://www.amazon.com/gp/product/B00T2U7R7I/ref=as_li_tl?ie=UTF8&camp=1789&creative=9325&creativeASIN=B00T2U7R7I&linkCode=as2&tag=gitlab-20&linkId=4QR3R64HBVB6T3TA). Make sure you have a new SIM card or phonenumber that does not actively use WhatsApp ready for this guide, you need this in step 4.*
...
* [Video tutorial](https://gitlab.maikel.pro/maikeldus/WhatsSpy-Public/wikis/how-to-setup-rpi-video)
...
* Fill in the empty fields in `$whatsappAuth` (`number` and `secret`). The `secret` is the thing you have retrieved in *chapter 4*.
   * `number` needs to be <countrycode><phonenumber> without any prefix 0's. *0031 06 120..* becomes *31 6 120..* (no 0's prefix for both the countrycode and phonenumber itself).
   * `number` may only contain digits. Spaces, plus or any other special character are NOT accepted. *Example: 316732174*
...
```
Check [this explanation about login failure](https://gitlab.maikel.pro/maikeldus/WhatsSpy-Public/wikis/troubleshooting#troubleshooting).

Another problem can be the fact that you have no internet connection, to check this execute `ping google.com` and check if there are responses. In case you do not get response you might need to [add nameservers](http://superuser.com/questions/617796/how-do-i-set-dns-servers-on-raspberry-pi) to your raspberry pi.
