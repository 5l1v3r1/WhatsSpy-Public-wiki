...
* If you want to log the tracker session to a log file you can call `` `which php` tracker.php | tee /var/log/tracker.log`` instead of step **3** in **Starting the tracker** (and you must install `tee`).
